"""Prompt builders for agent tasks."""
...
def build_prompt_for_task(task: AgentTask) -> str:
    """Build a prompt string for the given agent task.

    Delegates to task-type-specific builders.
    """
    builders = {
        "relevance_rerank": _build_relevance_prompt,
        "claim_extraction": _build_extraction_prompt,
        "review_synthesis": _build_review_prompt,
        "health_check": _build_health_check_prompt,
    }
...
def _build_health_check_prompt(task: AgentTask) -> str:
    return "Reply with the word READY and nothing else."


def _build_relevance_prompt(task: AgentTask) -> str:
    topic = task.topic or task.input_json.get("topic", "")
    papers = task.input_json.get("papers", [])

    paper_entries = []
    for p in papers:
        abstract = (p.get("abstract") or "")[:500]
        entry = f'{{"paper_id": "{p.get("paper_id", "")}", "title": "{p.get("title", "")}", "abstract": "{abstract}"}}'
...
def _build_extraction_prompt(task: AgentTask) -> str:
    topic = task.topic or task.input_json.get("topic", "")
    paper = task.input_json.get("paper", {})
    paper_id = task.paper_id or paper.get("paper_id", "")
    title = paper.get("title", "")
    abstract = paper.get("abstract", "") or task.input_json.get("source_text", "")
...
def _build_review_prompt(task: AgentTask) -> str:
    from knowcran.utils import citation_key

    topic = task.topic or task.input_json.get("topic", "")
    papers = task.input_json.get("papers", [])
    claims = task.input_json.get("claims", [])
    citation_keys_map = task.input_json.get("citation_keys", {})

    paper_refs = []
    for p in papers:
        key = citation_keys_map.get(p["paper_id"], citation_key(p))
    ...
    all_keys = list(set(citation_keys_map.values())) if citation_keys_map else []
